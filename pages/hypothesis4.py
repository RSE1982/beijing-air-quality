...
st.title(":material/hourglass: Hypothesis 4")

st.latex(r"""
    \begin{aligned}
H_0 &: \text{PM2.5 levels do not exhibit temporal structure.} \\
H_1 &: \text{PM2.5 levels show significant temporal patterns such as daily,
...
with col1:
    ...
    # Spearman correlations for temporal variables
    ...
    # correlation range
    corr_range = abs(
        results[strongest_var]["coef"]) - abs(
            results[weakest_var]["coef"])
...
with col2:
    tab = st.tabs([":material/hourglass: Hourly Variation",
                   ":material/calendar_today: Day of Week Variation",
                   ":material/calendar_month: Monthly Variation",
                   ":material/calendar_today: Yearly Variation",
                   ":material/analytics: Spearman Correlation"])
    with tab[0]:
    ...
    with tab[1]:
        ...
        with colB:
            ...
            **Why it matters:**
            ...
            industrial activity, and weekend behavioural changes on air
            quality.
    ...
    with tab[4]:
        ...
        with colA:
            st.subheader(":material/analytics:\
                          Spearman Correlations with Temporal Variables")
            results = []
            # Calculate Spearman correlations
            for var in time_vars:
                coef, p = spearmanr(df["pm25"], df[var])
                results.append({"Variable": var,
                                "Spearman ρ": coef, "p-value": p})
        ...
        with colB:
            ...
            **Why it matters:**
            ...
            trends across time-related cycles. It provides statistical
            confirmation of visible patterns.
